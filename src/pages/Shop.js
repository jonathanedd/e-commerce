...
const Shop = () => {
    ...
    const productFind = products.data?.products.find( productItem => productItem.id === Number(id) );
    // console.log(productFind?.category.id);

    //Filter products for category petition
    useEffect(() => {
        if(productFind){
            axios.get(`https://ecommerce-api-react.herokuapp.com/api/v1/products?category=${productFind?.category.id}`)
    ...
    return (
        <section className='shop-id'>
            ...
            <div className='shop-box-img'>
                <img width='300px' src={productFind?.productImgs} alt="" />
            </div>
            ...
            <ul className='product-list-filter'>
                {
                    filterProducts.data?.products.map( productItem => (
                        <li key={productItem.id}>
                            ...
                            <FontAwesomeIcon  className='second-add-button' icon={faCartShopping}/>
